...
function matchesCapabilities(provider, required) {
  return Object.entries(required).every(([capability, needed]) => !needed || Boolean(provider.capabilities?.[capability]));
}

export function redact(value) {
  return String(value)
    .replaceAll(redactionPatterns[0], '[REDACTED_OPENAI_KEY]')
    .replaceAll(redactionPatterns[1], '[REDACTED_GITHUB_TOKEN]')
    .replaceAll(redactionPatterns[2], '[REDACTED_GOOGLE_KEY]')
    .replaceAll(redactionPatterns[3], 'Bearer [REDACTED_TOKEN]');
}

export function validatePlatform() {
  const errors = [];
  for (const tier of ['PRIMARY', 'SECONDARY', 'DEGRADED']) {
    if (!providers.some((provider) => provider.tier === tier)) errors.push(`missing provider tier: ${tier}`);
  }
  for (const tool of tools) {
    ...
    if (!Number.isFinite(tool.timeoutMs) || tool.timeoutMs < 1000) errors.push(`timeout too low for ${tool.name}`);
  }
  for (const task of tasks) {
    if (!allowedStates.has(task.status)) errors.push(`invalid task state: ${task.id}`);
  }
...
export function selectProvider(required = { text: true }) {
  ...
  const candidates = [...providers]
    .filter((provider) => matchesCapabilities(provider, required))
    .sort((a, b) => (priority[a.tier] ?? 99) - (priority[b.tier] ?? 99));
  return candidates.find((provider) => provider.status === 'connected') ?? null;
}
...
export function planObjective(goal) {
  ...
  return {
    ...
    currentStep: clean
      ? 'Decompose goal, classify tool risk, then request only required permissions.'
      : 'Waiting for a concrete objective.',
    retries: 0,
...
export function summarizeDashboard() {
  return {
    ...
    credentialRequired: providers.filter((provider) => provider.status === 'needs_key').length,
    highRiskTools: tools.filter((tool) => tool.risk === 'high').length,
    activeTasks: tasks.filter((task) => !['COMPLETED', 'FAILED', 'CANCELLED'].includes(task.status)).length,
